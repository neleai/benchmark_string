...
/*A vectorized implementation of strcmp_dir is slower for small no.*/
...
static void two_way_preprocessing(uchar *n,size_t ns,size_t *per,size_t *ell,int *peri)
{
 int u,v,up,vp;
  int per,ell;
  u=maxSuf(n,ns,&up,0);
  ...
  ell = (u > v) ? u :  v;
  per = (u > v) ? up : vp;
  *peri = (strcmp_dir(n, n + per, ell +1) == ell+1) ;
  if (!*peri)
