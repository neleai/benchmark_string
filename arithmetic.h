...
inline MASKTYPE forget_bits(MASKTYPE m,int b){return (m >>(8*b))<<(8*b);}
...
inline MBTYPE get_mask(MBTYPE x) { return ((~x)&high_bit); }
inline MBTYPE GET_BIT(MBTYPE x,MBTYPE y) { return x&(((MBTYPE)1)<<(8*y+7)); } 
#define AND(x,y) (x|y)
#define OR(x,y)  (x&y)
