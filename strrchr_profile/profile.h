...
#define COMMON_MEASURE(fn,exp) \
  ...
  size_t r2= ((size_t)x+r)/16-((size_t)x)/16+1;\
	if (prof.fn.last-prof.fn.start<1000000){\
    if(r2<=1000) {\
...
/*	if (r==16) printf("%i\n",prof.fn.last-prof.fn.start);*/\
	if (r==16 && prof.fn.last-prof.fn.start>=0 && prof.fn.last-prof.fn.start<2000) prof.fn.time_distribution[choice][0][(prof.fn.last-prof.fn.start)/20]++;\
	else if (r==16) prof.fn.time_distribution[choice][0][99] += 1 + (prof.fn.last-prof.fn.start)/2000;\
	if (r==128 &&  prof.fn.last-prof.fn.start>=0 && prof.fn.last-prof.fn.start<2000) prof.fn.time_distribution[choice][1][(prof.fn.last-prof.fn.start)/20]++;\
	else if (r==128) prof.fn.time_distribution[choice][1][99] += 1 + (prof.fn.last-prof.fn.start)/2000;\
  if (b_##fn & B_NEEDLE){\
  if(ns<=1000)\
